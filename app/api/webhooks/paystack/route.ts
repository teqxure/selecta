...
/**
 * Paystack webhook — the only path that may move a Payment out of PENDING.
 * The reference we sent Paystack at checkout time is our own Payment id,
 * so `data.reference` here is exactly the row to look up. Signature
 * verification happens over the raw body, so it's read as text before any
 * JSON parsing — parsing and re-serializing would change the byte-for-byte
 * content Paystack signed.
 */
